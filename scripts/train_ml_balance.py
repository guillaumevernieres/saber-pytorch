#!/usr/bin/env python3
"""Train an ML balance FFNN surface emulator.
...
-----
    ...
    srun python scripts/train_ml_balance.py --config configs/aice.yaml

Workflow summary
----------------
1. Prepare training data (if not done yet):
       python scripts/prepare_training_data.py --config configs/aice.yaml \\
...
3. Export to TorchScript for SABER:
       python scripts/build_ml_balance_emulator.py \\
           --checkpoint models_aice/best_model.pt \\
           --output ml_balance_aice.ts
"""
